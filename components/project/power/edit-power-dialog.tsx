...
export function EditPowerDialog({ power, open, onOpenChange, onSuccess }: EditPowerDialogProps) {
    ...
    const form = useForm<PowerFormData>({
        ...
        defaultValues: {
            ...
            type: power.type || "special",
            rarity: power.rarity || "common",
            maxLevel: power.maxLevel || 10,
            icon: power.icon || "",
    ...
    const onSubmit = async (data: PowerFormData) => {
        ...
        const result = await updatePower(power.id, {
            name: data.name,
        ...
        if (result.success) {
            ...
            toast.error(result.error || "Failed to update power");
        }
