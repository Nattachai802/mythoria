...
> **Current Version: `v1.5`** — Rewrite Workspace, Context Fabric Architecture, Power System & Character Deep-Dive
...
- [🗺️ Roadmap: Context Fabric](#-roadmap-context-fabric)
...
### 9. Vector Search & RAG (Python Microservice)
...
| **AI** | `analysis_queue`, `suggestions`, `note_summaries`, `chapter_summaries`, `audit_issues` |
| **Sync** | `version_history`, `drive_sync_states` |
| **Auth** | `users`, `sessions`, `accounts`, `verifications` |

---

## 🗺️ Roadmap: Context Fabric

สถาปัตยกรรมรุ่นถัดไปที่กำลัง implement — เปลี่ยน Mythoria จากโมดูลที่เอกเทศน์ต่อกัน ให้เป็นระบบที่ทุก module เชื่อมหากันลื่นไหล:

```
L4  Context API ("MCP" gateway) — getContextFor / search / agent tools
L3  Knowledge Graph — ขยาย graph.ts → ครอบทุก entity ในโลก
L2  RAG / Vector — LanceDB ครอบทุก entity + auto-sync
L1  Reference Layer — 1 ตาราง แทน 20+ junction table
L0  Entity Registry — abstraction เหนือ 48 ตาราง
```

| Phase | ทำอะไร | สถานะ |
|---|---|---|
| 0 | Backup db + LanceDB | ✅ Done |
| 1 | L0 Entity Registry + L1 `references` table | 🔄 In Progress |
| 2 | Dual-write junction + reference | ⏳ Planned |
| 3 | Backfill + L4 Context API | ⏳ Planned |
| 4 | `@`-mention UI + RAG auto-sync | ⏳ Planned |
| 5 | ขยาย Knowledge Graph + Agent tools | ⏳ Planned |
